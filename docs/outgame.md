...
- 캐릭터 화면의 덱 관리 탭에서 언제든지 삭제할 수 있다.
...
| 버튼 | 이동 |
|------|------|
| **스토리** | 스토리 모드 (메인·서브 스토리) |
| **전투 모드** | 전투 모드 선택 (도전 로그라이크 · 일반 전투 · 도전 전투) |
| **캐릭터** | 캐릭터 화면 (캐릭터 정보 · 덱 관리 탭) |
| **종료** | 게임 종료 |

> 패키지 게임이므로 에너지·가챠·이벤트 배너 등 라이브 서비스성 요소는 메인 메뉴에 배치하지 않는다.

---

## 캐릭터 화면

보유 캐릭터 열람과 완성 덱 관리를 담당하는 화면. **캐릭터 정보** / **덱 관리** 두 탭으로 구성된다.

### 캐릭터 정보 탭

보유 캐릭터(기본 5인)의 정보를 확인하는 열람 전용 탭.
...
이 탭에서 직접 편집하거나 변경할 수 있는 요소는 없다.
...
- **일반 완성 덱** / **도전 완성 덱** 하위 탭으로 구분된다.
- 각 하위 탭에서 최대 10개 덱을 저장·관리한다.
